...
export default function CSVReader() {
  ...
  const [startDate, setStartDate] = useState(null);
  const [endDate, setEndDate] = useState(null);
  const [speedRange, setSpeedRange] = useState([0, 20]);
  ...
  const displayedData = csvData && (startDate || endDate || speedRange)
    ? csvData.filter(row => {
        const rowDate = parseDate(Number(row["day"]), Number(row["month"]), Number(row["year"]));
        if (startDate && endDate && (!rowDate || rowDate < startDate || rowDate > endDate)) return false;
        if (startDate && (!rowDate || rowDate < startDate)) return false;
        if (endDate && (!rowDate || rowDate > endDate)) return false;
        const speed = parseFloat(row["ws100"]);
        if (speed < speedRange[0] || speed > speedRange[1]) return false;
        return true;
      })
      : csvData;
  ...
  return (
    <>
      ...
      >
        {({ getRootProps, acceptedFile, ProgressBar, getRemoveFileProps }) => (
          <>
            ...
            <div className="csv-info-message">
              * O tempo de execução do sistema varia de acordo com a quantidade de dados, aguarde um momento ao inserir seu arquivo e ao utilizar  os filtros
            </div>
          </>
        )}
      </CSVReader>
      {!csvData && <Inicio />}
      {csvData && (
        <div className="csv-filters-box">
          <div className="csv-title">
            Potencial de Geração de Energia Eólica
          </div>
          <div className="csv-filters-row">
            <div className="csv-filter-box">
              <label className="csv-filter-label">
                Filtrar por Data:
              </label>
              <span style={{ position: "relative", display: "inline-block" }}>
                <span
                  className="csv-info-icon"
                  title="Selecione o período desejado para análise. Clique primeiro na data inicial."
                >
                  i
                </span>
              ...
              <DatePicker
                ...
                popperPlacement="bottom"
                calendarClassName="modern-datepicker"
                className="csv-datepicker"
              />
            </div>
            <div className="csv-filter-box">
              <label className="csv-filter-label-vel">
                Filtrar por Velocidade (m/s):
              </label>
              <span style={{ position: "relative", display: "inline-block" }}>
                <span
